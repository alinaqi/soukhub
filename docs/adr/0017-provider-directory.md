...
- **Conversion path** (implemented): "Claim my store" on a shop's page turns the listing into a full SoukHub seller org in one click — the claim endpoint provisions the org from the session user, prefills a fresh store with the shop's public identity (name, logo, area bio), and stamps `claimed_org_id`/`claimed_at` with a first-write-wins null guard (later claimants get 409). Guests round-trip through login with an allowlist-validated `next` path and the claim auto-resumes. Claimed shops carry an "On SoukHub" badge.
- **Review texts**: ingestion stores up to 5 top Google reviews per shop (`providers.google_reviews`, clamped author/text/stars) — shown as review cards on detail pages and emitted as schema.org `Review` objects next to the aggregate rating.
