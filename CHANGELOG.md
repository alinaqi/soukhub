...
- **Auto-generated promo banners (Protaige Sketch)**: on-brand campaign banners are generated from the retail calendar via Sketch's REST API, persisted to Supabase Storage, and shown on the home page for the live event (Back to School → laptop banner today). `pnpm banners:generate` produces the active + next events; `promo_banners` table with public-read RLS and a public storage bucket; graceful fallback to the text banner when no image exists
- **AI timing advice from the retail calendar**: the shopping agent has a `get_event_calendar` tool — for "buy now or wait?" questions it checks upcoming UAE sales (Back to School, White Friday, DSF, Ramadan…) and gives grounded, honest advice ("White Friday is ~2 weeks away — may be worth waiting"), never inventing a discount it can't back up
...
- **AI agent spec alignment (ADR 0018)**: assistant answers lead with one "Get this one" pick plus at most two plain-language alternatives (hard 3-product cap), ≤3-question policy, honest no-match behaviour; home hero asks "What do you need?" with an Ask AI hand-off; catalog pages show "Price verified {n}h ago"; full gap report in docs/spec-gap-report.md
